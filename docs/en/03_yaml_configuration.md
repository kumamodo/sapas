...
| `LINK` | Dict | Defines connection drivers (e.g. SSH, ADB, UDP, UART). Contains sub-parameters such as `type`, `host`, `user`, `password`, `source_ip` (optional local PC NIC source IP), `drain_timeout` (optional UDP packet chunk drain timeout). The lowercase `link` key is deprecated. |
